...
const MarksManagement: React.FC<MarksManagementProps> = ({ userRole, userId, teacherId }) => {
  ...
  const searchStudent = async () => {
    ...
    try {
      ...
      await loadMarksForStudent(data.id, selectedExam);
  ...
  const loadMarksForStudent = async (studentId: string, examType: ExamType) => {
    try {
      const { data, error } = await supabase
        .from('marks')
        .select(`
    ...
    setSubjectMarks(prev =>
      prev.map(mark =>
        mark.subjectId === subjectId
          ? { ...mark, [field]: field === 'marksObtained' ? Math.min(100, Math.max(0, Number(value) || 0)) : value }
          : mark
      )
    );
  };
  ...
  const calculateOverallPercentage = () => {
    ...
    const total = subjectMarks.reduce((sum, mark) => sum + mark.marksObtained, 0);
                ...
                <>
                  ...
                  <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="text-sm text-gray-500">All marks are out of 100</div>
                    </div>
                          ...
                          <div className="grid grid-cols-12 gap-6 items-center">
                            <div className="col-span-3">
                              <div className="flex items-center gap-3">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${getGradeColor(mark.marksObtained)}`}>
                                  <BookOpen className="w-6 h-6" />
                                </div>
                                <div>
                                  <div className="font-semibold text-gray-900">{mark.subjectName}</div>
                                  <div className={`text-sm font-medium ${getGradeColor(mark.marksObtained)}`}>
                                    {mark.marksObtained}%
                                  </div>
                                </div>
                              </div>
